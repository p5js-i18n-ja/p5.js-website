...
- ファイルの依存関係は正しいですか？
  ...
  基本的なスケッチは、コンピューターにインストールされているか、p5.jsプロジェクトのHTMLファイルにリンクされている[p5.jsのバージョン](https://p5js.org/download/)に依存しています。[他のライブラリを使用する](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Understanding_client-side_tools/Package_management)場合は、それらがHTMLファイルにリンクされているか、コンピューターに適切に保存されていることを確認してください。
...
プログラマーは、最後の引数と セミコロンの間に括弧を追加することで問題を解決しました。彼らは[`rect()`](https://p5js.org/reference/#/p5/rect)のp5.jsリファレンスページを訪れ、コードを比較することでこれを発見しました。

#### プロのヒント：

1. 常に[p5.jsリファレンス](https://p5js.org/reference/)を参照して、コードが正しい構文で書かれていることを確認してください。
...
コンソールのメッセージによると、[`circle()`](https://p5js.org/reference/#/p5/circle)は3つの入力を期待していたのに対し、2つしか受け取りませんでした。メッセージはまた、エラーがスケッチの9行目にあることを指定しています。
...
プログラマーは、[`circle()`](https://p5js.org/reference/#/p5/circle)に3つ目の引数を追加することで問題を解決しました。

彼らは[`circle()`](https://p5js.org/reference/#/p5/circle)のp5.jsリファレンスページを訪れ、コードを比較することでこれを発見しました。
...
プログラマーは、11行目に*閉じ中括弧*（`}`）を追加して[`draw()`](https://p5js.org/reference/#/p5/draw)関数を閉じることで問題を解決しました。

彼らは[`draw()`](https://p5js.org/reference/#/p5/draw)のp5.jsリファレンスページを訪れ、コードを比較することでこれを発見しました。
...
コンソールのメッセージによると、プログラマーは10行目でエラーを犯しています - [circle()](https://p5js.org/reference/#/p5/circle)関数は最初のパラメータとして[数値](https://p5js.org/reference/#/p5/number)を必要としていますが、代わりに[文字列](https://p5js.org/reference/#/p5/string)が与えられました。
...
プログラマーは、変数`myX`が`"forty-five"`で初期化されていることに気づきました。これは[数値](https://p5js.org/reference/#/p5/number)として認識されません。p5.jsはこれを[文字列](https://p5js.org/reference/#/p5/string)データ型の単なる単語だと考えています。
...
プログラマーは、[`setup()`](https://p5js.org/reference/#/p5/setup)と[`draw()`](https://p5js.org/reference/#/p5/draw)の外部で`localVariable`を宣言し、[`setup()`](https://p5js.org/reference/#/p5/setup)内で初期化することで問題を解決しました。
...
- [p5.jsのダウンロード](https://p5js.org/download/)
